...
class WebSocketHandler:
    ...
    async def send(self, message: Dict) -> None:
        ...
        if message["type"] == "websocket.accept":
            ...
            with open("WSclientserver/TestFileReceived.txt", "w") as fp:
                fp.write(str(message["text"]))

            if message.get("text") is not None:
                data = self.websocket.send(  # Send ACK to be used for tests
                    wsproto.events.TextMessage("0")
                )
...
Handler = Union[WebSocketHandler, WebTransportHandler]

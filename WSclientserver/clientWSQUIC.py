...
def save_session_ticket(ticket: SessionTicket) -> None:
    if args.session_ticket:  # qua non c'è bisogno che lo pubblico, è ticket resumption
...
async def run(
    ...
    async with connect(
        ...
        with open("WSclientserver/TestFileToSend.txt") as f:
            message = f.read()
...
if __name__ == "__main__":
    ...
    if args.session_ticket:
        try:
            file = open(args.session_ticket, "rb")
            configuration.session_ticket = pickle.load(file)
            file.close()
